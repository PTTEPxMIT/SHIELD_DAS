...
class Dataset:
    ...
    Attributes:
        ...
        sample_coating: Human-readable coating description (e.g.
            "800nm tungsten"); "none" for an uncoated sample
        sample_coating_layers: Coating layers as a list of dicts with
            "material" and "thickness_nm" keys (empty list if uncoated)
    ...
    def metadata(self):
        ...
        # Validate version - support 1.3 (sample_material) and 1.4
        # (sample_substrate + sample_coating)
        version = metadata.get("version")
        if version not in ("1.3", "1.4"):
            raise ValueError(
                f"Unsupported metadata version: {version}. "
                f"Only versions 1.3 and 1.4 are supported. "
    ...
    def process_data(self):
        ...
        # sample_material, which maps onto the substrate with no coating info.
        run_info = self.metadata["run_info"]
        self.sample_substrate = run_info.get(
            "sample_substrate", run_info.get("sample_material", "Unknown")
        )
