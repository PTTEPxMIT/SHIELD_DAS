...
1. Open `record_run.py` and edit the settings in the **"EDIT ME"** block at the
   top: furnace setpoint (°C), the sample description (substrate, coating,
   thickness in m, and its `sample_id`), run type, and recording/backup
   intervals. The gauge and thermocouple wiring lower down only needs changing
   if the rig wiring changes.
...
3. During the run, press **SPACEBAR** at each valve event. The recorder prompts
   ...
   each timestamp in the run metadata. (A leak test has a single event
   instead — see [Leak test runs](#leak-test-runs).)
